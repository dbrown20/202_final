    ...
    def tc_exp(e: Expr, env: TEnv) -> type:
        match e:
            ...
            case Begin(stmts, e):
                ...
                return tc_exp(e, env)
            case Call(func, args):
                func_t = tc_exp(func, env)
                match func_t:
                    case Callable(args_t, return_type):
                        for a1, a2 in zip(args, args_t):
                            assert tc_exp(a1, env) == a2
                        return return_type
                    case _:
                        raise Exception('tc_exp', e)
            case _:
...
def rco(prog: Program) -> Program:
    ...
    def rco_exp(e: Expr, bindings: Dict[str, Expr]) -> Expr:
        match e:
            ...
            case Call(func, args):
                new_args = [rco_exp(e, bindings) for e in args]
                new_func = [rco_exp(e, bindings) for e in func]

                new_e = Prim(new_func, new_args)
                new_v = gensym('tmp')
                bindings[new_v] = new_e
                return Var(new_v)
            case _:
...
def explicate_control(prog: Program) -> cfun.CProgram:
    ...
    def ec_function(name: str, params: List[Tuple[str, type]], body_stmts: List[Stmt], return_type: type):
        ...
        # set basic_blocks to {} and current_function to the functions name
        basic_blocks = {}
        current_function = name

        # call ec_stmts on the body statements, with the continuation Return(0)
        explicate_stmts(body_stmts, Return(0))
        # set basic_blocks[name + 'start'] to the result of 3
        basic_blocks[name + 'start'] = explicate_stmts(body_stmts, Return(0))

        # construct a cfun.FunctionDef with the name, param names, and basic_blocks
        param_names = [tup[0] for tup in params]
        func = cfun.CFunctionDef(name, param_names, basic_blocks)

        # append the function def to functions
    ...
    def explicate_exp(e: Expr) -> cfun.Expr:
        match e:
            case Call(func, args):
                ...
                return cfun.Call(func, new_args)
            case Prim(op, args):
                new_args = [explicate_atm(a) for a in args]
    ...
    def explicate_stmt(stmt: Stmt, cont: List[cfun.Stmt]) -> List[cfun.Stmt]:
        match stmt:
            ...
            case While(Begin(condition_stmts, condition_exp), body_stmts):
                whileLabel = current_function + 'loop_label'
                cont_label = create_block(cont)
                test_label = gensym(whileLabel)
...
def select_instructions(prog: cfun.CProgram) -> X86ProgramDefs:
    ...
    def si_stmt(stmt: cfun.Stmt) -> List[x86.Instr]:
        match stmt:
            ...
            case cfun.Call(fun, args):
                # todo
                # fill in from question 2
                return [x86.IndirectCallq(si_atm(fun), 0),
                        x86.NamedInstr('movq', [x86.Reg('rax'), x86.Var(args)])]
    ...
    def si_def(d: cfun.CFunctionDef) -> X86ProgramDefs:
        # todo fill in using question 2
        ...
        return X86ProgramDefs(d.name, d.blocks, (None, None))
...
def allocate_registers(program: X86ProgramDefs) -> X86ProgramDefs:
    ...
    for definition in program.defs:
        miniProg = x86.X86Program(definition.blocks)
        miniProgResult = _allocate_registers(definition.label, miniProg)
        newDef = X86FunctionDef(definition.label, miniProgResult.blocks, miniProgResult.stack_space)
        newDefs.append(newDef)

    newProg = X86ProgramDefs(newDefs)
    return newProg
...
def patch_instructions(program: X86ProgramDefs) -> X86ProgramDefs:
    ...
    """
    # call _patch_instructions for each definition
    newDefs = []
    for definition in program.defs:
        miniProg = x86.X86Program(definition.blocks)
        miniProgResult = _patch_instructions(miniProg)
...
def prelude_and_conclusion(program: X86ProgramDefs) -> x86.X86Program:
    ...
    """
    # call _prelude_and_conclusion for each definition
    newBlocks = {}
    for definition in program.defs:
        miniProg = x86.X86Program(definition.blocks)
        miniProgResult = _prelude_and_conclusion(definition.label, miniProg)
        newBlocks.update(miniProgResult.blocks)

    newProg = x86.X86Program(newBlocks)
    return newProg
